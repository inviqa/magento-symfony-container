...
class Inviqa_SymfonyContainer_Helper_ServiceInjector
{
    ...
    {
        $dependables = $this->getService(InjectableCompilerPass::INJECTABLES_SERVICE_ID)->dependables;

        if (!isset($dependables[get_class($class)])) {
            return;
        }
        ...
        $services = $this->getServices($dependables[get_class($class)]);
